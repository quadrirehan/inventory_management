...
// only have a single app-wide reference to the database
  ...
  Future<void> _onCreate(Database db, int version) {
    ...
    db.execute(
        "CREATE TABLE stock_transaction(stock_id	INTEGER PRIMARY KEY AUTOINCREMENT, ledger_id	INTEGER, product_id	INTEGER, voucher_type	TEXT, voucher_number	INTEGER, stock_in	TEXT, stock_out	TEXT, total_stock TEXT)");
  }
  ...
  Future<void> addToPurchase(
      int _ledgerId, int _productId, String _purchaseTotal, String _gst, String _grandTotal, String _stockIn) async {
    //TODO: RAW INSERT QUERY
    Database db = await instance.database;
    db.rawInsert(
        "INSERT INTO purchase(ledger_id, product_id, purchase_total, purchase_gst, purchase_grand_total) VALUES('$_ledgerId', '$_productId', '$_purchaseTotal', '$_gst', '$_grandTotal')").then((value) {
      db.rawQuery("select last_insert_rowid() FROM purchase").then((_purchaseId) {
        stockTransaction(_ledgerId, _purchaseId[0]['last_insert_rowid()'], _productId, "Purchase", _stockIn, "0").then((_stockTId) {
          db.rawUpdate("UPDATE purchase SET stock_t_id = '${_stockTId[0]['last_insert_rowid()']}' WHERE purchase_id = '${_purchaseId[0]['last_insert_rowid()']}'");
          db.rawQuery("SELECT product_qty FROM products WHERE product_id = $_productId").then((_productQty) {
            ...
            db.rawUpdate("UPDATE products SET product_qty = '$qty' WHERE product_id = '$_productId'");
          });
        });
      });
    });
  ...
  Future<List> stockTransaction(int _ledgerId, int _voucherNo, int _productId, String _voucherType, String _stockIn, String _stockOut) async {
    Database db = await instance.database;
    db.rawInsert("INSERT INTO stock_transaction(ledger_id, voucher_number, product_id, voucher_type, stock_in, stock_out) VALUES('$_ledgerId', '$_voucherNo', '$_productId', '$_voucherType', '$_stockIn', '$_stockOut')");
    return db.rawQuery("select last_insert_rowid() FROM stock_transaction");
  }

  Future<void> addToSell(
      int _ledgerId, int _productId, String _sellTotal, String _sellGst, String _grandTotal, String _stockOut) async {
    //TODO: RAW INSERT QUERY
    Database db = await instance.database;
    // db.rawInsert(
    //     "INSERT INTO purchase_items(vendor_id, product_id, p_item_price, p_item_qty) VALUES('$vendorId', '$p_id', '$p_price', '$p_qty')");
    db.rawInsert(
        "INSERT INTO sell(ledger_id, product_id, sell_total, sell_gst, sell_grand_total) VALUES('$_ledgerId', '$_productId', '$_sellTotal', '$_sellGst', '$_grandTotal')").then((value) {
      db.rawQuery("select last_insert_rowid() FROM sell").then((_sellId) {
        stockTransaction(_ledgerId, _sellId[0]['last_insert_rowid()'], _productId, "Sell", "0", _stockOut).then((_stockTId) {
          db.rawUpdate("UPDATE sell SET stock_t_id = '${_stockTId[0]['last_insert_rowid()']}' WHERE sell_id = '${_sellId[0]['last_insert_rowid()']}'");
          db.rawQuery("SELECT product_qty FROM products WHERE product_id = $_productId").then((_productQty) {
            print(_productQty[0]['product_qty']);
            double qtyTemp = double.parse(_productQty[0]['product_qty']);
